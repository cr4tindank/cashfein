...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        ...
        button_furniture = tk.Button(frame, text="Furniture", font=("Helvetica", 12), compound=tk.TOP, bg="green", fg="white", padx=20, pady=10, command=lambda: self.show_data('furniture'))
        button_furniture.grid(row=0, column=0, padx=20)

        button_ukuran = tk.Button(frame, text="Ukuran", font=("Helvetica", 12), compound=tk.TOP, bg="purple", fg="white", padx=20, pady=10, command=lambda: self.show_data('ukuran'))
    ...
    def tambah_furniture(self):
        ...
        if nama_furniture.strip() == "" or id_ukuran.strip() == "" or id_warna.strip() == "":
            messagebox.showerror("Error", "Nama ukuran, ukuran, dan warna tidak boleh kosong!")
        else:
            try:
